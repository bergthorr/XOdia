...
#include <stdlib.h>
#include <ctime>
//need to change one condition in is_danger if(
using namespace std;
...
void merge_leaf(int r, int c) {
	...
	if (r < 19 && r > 0 && c < 19 && c > 0) {
		if (array[r - 1][c][0] == LEAF_MY
				|| (array[r - 1][c][0] == EMPTY && is_danger(r, c, 1, r - 1, c))) {
			i2 = r - 1;
			j2 = c;
			points++;
		}
		...
		if (array[r][c + 1][0] == LEAF_MY
				|| (array[r][c + 1][0] == EMPTY && is_danger(r, c, 3, r, c + 1))) {
			if (i2 == 99) {
				i2 = r;
				j2 = c + 1;
				points++;
		...
		if (array[r + 1][c][0] == LEAF_MY
				|| (array[r + 1][c][0] == EMPTY && is_danger(r, c, 5, r + 1, c))) {
			if (i2 == 99) {
				i2 = r + 1;
				j2 = c;
				points++;
		...
		if (array[r][c - 1][0] == LEAF_MY
				|| (array[r][c - 1][0] == EMPTY && is_danger(r, c, 7, r, c - 1))) {
			if (i2 == 99) {
				i2 = r;
				j2 = c - 1;
				points++;
...
//
void check_forward_growth1(int r, int c) {
	// s s
	// a s
	// s s
...
//////////////////
	...
	//to move forward
	for (int column = 8; column >= 7; column--) {
		for (int row = 6; row < 15; row++) {
			if (array[row][column][0] == LEAF_MY) {
				if (point == 1) {
					r1 = row;
	...
	//to move forward
		for (int column = 18; column >= 0; column--) {
			...
			if (found_point) {
				...
				if (r2 != 99) {
					if (abs(r1 - 9) < abs(r2 - 9)) {
						check_forward_growth1(r1, c1);
						check_forward_growth1(r2, c1);
					} else {
						check_forward_growth1(r2, c1);
						check_forward_growth1(r1, c1);
					}
			...
			for (int z = 0; z < check_index; z = z + 2) {
				check_forward_growth1(check[z], check[z + 1]);
			}
			check_index = 0;
		}
		//to move forward end 2


	//make a move ...new or merge
	...
	//add random bot to avoid any remaining move
	int x[400];
			int y[400];
